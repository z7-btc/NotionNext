...
function filterPostBlocks(id, blockMap, slice) {
  ...
  for (const i in clonePageBlock?.block) {
    ...
    if (
      ...
      const sourceUrl = b?.value?.properties?.source?.[0][0]
      if (sourceUrl) {
        // 处理新格式的attachment URL
        if (sourceUrl.startsWith('attachment:')) {
          console.log('Processing attachment URL in filterPostBlocks:', sourceUrl)
          
          // 如果block中已经有完整的URL，直接使用它
          if (b?.value?.format?.display_source && !b.value.format.display_source.startsWith('attachment:')) {
            console.log('Using original URL:', b.value.format.display_source)
            b.value.properties.source[0][0] = b.value.format.display_source
            continue
          }
          
          // 如果block中有source字段且包含完整URL，使用它
          if (b?.value?.properties?.source?.[0]?.[0] && !b.value.properties.source[0][0].startsWith('attachment:')) {
            console.log('Using original source URL:', b.value.properties.source[0][0])
            continue
          }
          
          // 否则尝试从block的其他属性中查找原始URL
          const originalUrl = b?.value?.format?.display_source || b?.value?.properties?.source?.[0]?.[0]
          if (originalUrl && !originalUrl.startsWith('attachment:')) {
            console.log('Found original URL in block:', originalUrl)
            b.value.properties.source[0][0] = originalUrl
            continue
          }
          
          // 如果找不到原始URL，保持attachment格式
          console.log('No original URL found, keeping attachment format')
          continue
        } else if (sourceUrl.indexOf('amazonaws.com') > 0) {
          // 处理旧格式的URL
          const newUrl = `https://notion.so/signed/${encodeURIComponent(sourceUrl)}?table=block&id=${b?.value?.id}`
