...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  if (ret?.startsWith('attachment:')) {
    console.log('Processing attachment URL:', ret, 'Block:', block) // 添加更多日志
    
    // 如果block中已经有完整的URL，直接使用它
    if (block?.format?.display_source && !block.format.display_source.startsWith('attachment:')) {
      console.log('Using original display_source URL:', block.format.display_source)
      return block.format.display_source
    }

    // 如果block中有source字段且包含完整URL，使用它
    if (block?.properties?.source?.[0]?.[0] && !block.properties.source[0][0].startsWith('attachment:')) {
      console.log('Using original source URL:', block.properties.source[0][0])
    ...
    if (fileName?.endsWith('.mp4')) {
      console.log('No original URL found, skipping video processing')
      return ret // 返回原始的attachment URL，让getPostBlocks处理它
    }
    
    // 处理图片文件
    if (fileName?.match(/\.(jpg|jpeg|png|gif|webp)$/i)) {
      const spaceId = '17b0a3b0-9813-479b-97d8-912500276264'
      const baseUrl = `https://img.notionusercontent.com/s3/prod-files-secure/${spaceId}/${fileId}/${fileName}`
      
      const width = block?.format?.block_width || 670
      const params = new URLSearchParams({
        exp: getExpirationTimestamp().toString(),
        w: width.toString()
      })
      
      return `${baseUrl}/size/w=${width}?${params.toString()}`
    }
