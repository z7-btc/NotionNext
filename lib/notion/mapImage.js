...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  if (ret?.startsWith('attachment:')) {
    ...
    if (fileName?.endsWith('.mp4')) {
      // 视频文件
      ret = `https://file.notion.so/f/f/17b0a3b0-9813-479b-97d8-912500276264/${fileId}/${fileName}?table=block&id=${block.id}&spaceId=17b0a3b0-9813-479b-97d8-912500276264&expirationTimestamp=1738180800000`
    } else {
      // 图片文件
      ret = `https://file.notion.so/f/f/17b0a3b0-9813-479b-97d8-912500276264/${fileId}/${fileName}?table=block&id=${block.id}&spaceId=17b0a3b0-9813-479b-97d8-912500276264`
    }
  }
  ...
  const hasConverted =
    ...
    ret.indexOf('https://file.notion.so/f/f/') === 0 ||
