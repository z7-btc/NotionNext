...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  if (ret?.startsWith('attachment:')) {
    const attachmentId = ret.split(':')[1]
    const fileName = ret.split(':')[2]
    // 构建新的URL格式
    if (fileName?.endsWith('.mp4')) {
      // 视频文件
      ret = `https://www.notion.so/signed/video/${attachmentId}/${fileName}`
    } else {
      // 图片文件
      ret = `https://www.notion.so/signed/image/${attachmentId}/${fileName}`
    }
  }
